...
public class DriveTrainV2 {
	...
	private TrajectoryDriveController pRightRightSwitchSide, pRightLeftSwitchBack, pRightLeftSwitchFront, pRightRightScale, pRightLeftScale;
	private TrajectoryDriveController pInUse;
	
	private enum Mode_Type {TELEOP,AUTO_SPLINE, AUTO_BACKWARDS_SPLINE, AUTO_MIRROR_SPLINE, AUTO_MIRROR_BACKWARDS_SPLINE, AUTO_DRIVE, AUTO_PATHFINDER, AUTO_MIRROR_PATHFINDER};
	...
	private PathfinderFollower p_right_rightScale, p_right_leftScale;
	private PathfinderFollower p_in_use, p_test, pRightRightSwitchFront, pLeftScaleRightSwitch, pRightScaleLeftSwitch, pRightSwitchBlock, pRightScaleCube, pSecondScaleCube, pRightScaleBackwards, pRightScaleCubeBack;
	...
	public DriveTrainV2() {
		...
		/*	pStraight = new TrajectoryDriveController("/home/lvuser/AutoFiles/Straight.txt", 0.0, 0, 0, 1.0/11.5, 1.0/50.0, -0.05);
			...
			*/
			...
			DriverStation.reportError("Spline: All auto files not on robot!", false);
		}
	...
	public void auto_right_rightScale() {
		resetEncoders();
		p_in_use = p_right_rightScale;
		p_in_use.reset();
		mode = Mode_Type.AUTO_PATHFINDER;
	}
	
	public void auto_right_leftScale() {
		resetEncoders();
		p_in_use = p_right_leftScale;
		p_in_use.reset();
		mode = Mode_Type.AUTO_PATHFINDER;
	}
	...
	public void update() {
		...
		switch(mode) {
			...
			case AUTO_PATHFINDER:
				double[] p = p_in_use.getOutput(rightEncoder.getDistance(), leftEncoder.getDistance(), getAngle()*Math.PI/180);
			...
			case AUTO_MIRROR_PATHFINDER:
				...
				p_mirror = p_in_use.getOutput(leftEncoder.getDistance(), rightEncoder.getDistance(), -getAngle()*3.14/180); 
